let stock=JSON.parse(localStorage.getItem('stock')) || [];
    ...
    function agregarMedia(){
        ...
        let precio=0;
        if (modelo==="Soquete" || modelo==="Soquete niño"){
            precio=1500;
        } else if(modelo==="3/4"){
            precio=2000;
        }

        stock.push({modelo, nombre, cantidad, ubicacion, precio});
        guardarStock();
        sincronizarConSheets('stock');
        renderizarStock();
...
const URL_SHEETS='https://script.google.com/macros/s/AKfycbzpAP32NtgvvK3hj_PDE5EDFdyPpEKZNolFo48RZoEv14_DE5QMtiDnNA9DwFHI165zVA/exec';

function sincronizarConSheets(tipo){
    let datos=[];
     if (tipo==='stock'){
        datos=stock;
     } else if(tipo==='ventas'){
     ...
     .then(res=>res.text())
     .then(respuesta=>{
        console.log(`✅${tipo} sincronizado con Google sheets.`);
     })
     .catch(err=>{
        console.error(`❌ Error al sincronizar ${tipo}:`, err);
     });
}